...
class App:
    ...
    def login(self):
        ...
        # 👉 2) Require blinking (เพิ่มตรงนี้)
    ...
    def require_blink(self):
        self.msg_box("Liveness Check", "Please blink once to verify you are real.")

        blinked = False
        start = time.time()

        while time.time() - start < 3:  # 3 seconds to blink
            ret, frame = self.cap.read()
        ...
        if not blinked:
            util.msg_box("Failed", "No blink detected. You might be a spoof!")
        return blinked
